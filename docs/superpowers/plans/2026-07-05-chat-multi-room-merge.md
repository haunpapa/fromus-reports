...
Expected: FAIL — `find_inputs` 미정의 (기존 `test_arg_priority_and_prefix`는 PASS 유지)
...
핵심 규칙(spec §4.3): 방별 최신 스냅샷을 **dedup 없이 그대로** 채택(base) → 같은 분 동일 발화 반복 보존. 같은 태그의 다른 파일에서 **내용 키 `(date,time,sender,body)` + 파일내 occ**가 base에 없는 메시지만 fold-in. fold가 발생한 태그만 `(date,time)` 안정 정렬. 태그 그룹을 최초등장 datetime 순으로 concat 후 전역 idx 재부여.
...
Run (두 번 다른 해시시드로 생성해 동일 바이트인지):
```bash
cd /Users/haunpapa/Documents/GitHub/fromus-reports
F="$HOME/Downloads/$(ls -t ~/Downloads | grep '^KakaoTalk_Chat.*프롬어스.*\.csv$' | head -1)"
PYTHONHASHSEED=1 python generator/update_archive.py --no-resolve "$F" < /dev/null >/dev/null; cp chat_kb.json /tmp/kb1.json
...
git checkout chat_kb.json   # 산출물 되돌리기(커밋 금지)
```
Expected: `DETERMINISTIC` (diff 없음)
...
```bash
git add generator/chat_to_kb.py
git commit -m "fix: chat_kb.json set 반복 sorted()로 결정론 고정"
```
...
- [ ] `python generator/test_parse.py -v` 전부 PASS (기존 + 신규 `TestRoomOf`/`TestMerge`/`TestRoomGuards`/`TestFindInput` 확장)
