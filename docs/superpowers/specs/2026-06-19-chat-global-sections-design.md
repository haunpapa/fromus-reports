...
- **점프 칩 바 상단 고정**: `position:sticky; top:0; z-index` — 전략·목표가·액션·뉴스·교육·Q&A 앵커(`href="#chat-<sec>"` 또는 스크롤). 뷰 안 스크롤 컨테이너 기준으로 고정.
- 섹션 순서: 🧭 전략(374) → 🎯 목표가(81) → 💡 액션(635) → 📰 뉴스(1,467) → 📚 교육(33) → ❓ Q&A(6).
- 각 섹션: 기존 `sec-title` 패턴 + 초기 N개 + **더보기 10개씩**. 모든 리스트 date 역순 정렬(렌더 시 클라이언트에서).
...
| 목표가 | stock,value,unit,date,sharer | 행. 종목은 `data-stock`(클릭→종목탭, 기존 L1071 핸들러), 값/발언자/날짜 |
| 액션 | kind,text,date,sharer | `kind`(do/dont 등) **색/아이콘 구분** 리스트 |
...
- 더보기: 1단계와 동일하게 `data-*` 속성 + 위임 핸들러. 기존 `data-stock`/`data-chat-*`와 충돌 없는 새 속성(예: `data-cg-sec`, `data-cg-shown`).
- 목표가 종목 칩은 `data-stock` 재사용(클릭 시 종목 탭 이동 — 의도된 동작).
- 정렬은 렌더 시 클라이언트에서(`D.chat` 그대로 → merge_hub 무변경).
...
- **점프 칩 sticky**: 뷰의 스크롤 컨테이너/기존 헤더와 겹침·z-index 주의. `window.scrollTo`(L661)와 앵커 점프 상호작용 확인.
- **더보기 속성 충돌**: 기존 `data-stock`(L1071)·`data-chat-*`(1단계)와 다른 접두사(`data-cg-*`) 사용.
